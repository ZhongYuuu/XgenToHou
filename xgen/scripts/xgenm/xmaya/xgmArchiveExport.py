...
class xgmArchiveExport(object):
    ...
    def filterManualLodNodes( self, objs, lodLo, lodMed ):
        baseObjs = []
        for obj in objs:
            if obj.endswith( lodLo ) or obj.endswith( lodMed ):
                continue
            baseObjs.append( obj )
    ...
    def processFile( self, f, objs, lodMode, lodLo, lodMed, startFrame, endFrame ):
        ...
        # batchMode 
        if ( objs == None or len(objs) == 0 ) and lodMode == 2:
            # Open Scene
            self.openScene( oriSceneFile )
            objs = self.fixNodeToShapeParent(objs)
            objs = self.filterManualLodNodes( objs, lodLo, lodMed )

        # Count the number of sub steps
        validLodMed = False
        validLodLo = False
        if lodMode==1:
            validLodMed = float(lodMed)>0.0
            validLodLo = float(lodLo)>0.0
        elif lodMode==2:
