...
errWrapXgenDesc = maya.stringTable[u'y_xgenm_xmaya_xgmExternalAPI.kXgmExtAPIErrWrapXgenDesc' ]
errWrapXgenDeleteDesc = maya.stringTable[u'y_xgenm_xmaya_xgmExternalAPI.kXgmExtAPIErrWrapXgenDeleteDesc' ]
...
def _createDescription( palette, description, primitive, generator, renderer, method, grooming ):
    ...
    try:
        ...
        xglog.XGError(errWrapXgenDesc + newDesc)
        xglog.XGError(errWrapXgenDeleteDesc)
...
def setActive( palette, description, subtype, previewer=False ):
    ...
    if description=="":
        xglog.XGError(maya.stringTable[u'y_xgenm_xmaya_xgmExternalAPI.kXgmExtAPICouldNotSetActiveType' ]+" ["+palette+":]");
        return
    cmds.file(modified=True)
...
def initInterpolation( palette, description, fileName="" ):
    ...
    # on the maya guides.
    if description=="":
        xglog.XGError(maya.stringTable[u'y_xgenm_xmaya_xgmExternalAPI.kXgmExtAPICouldNotSetInitInterop' ]+" ["+palette+":]");
        return False
    ...
    try:
        ...
        xglog.XGError(maya.stringTable[u'y_xgenm_xmaya_xgmExternalAPI.kXgmExtAPIExceptionXgmInterpSetup' ])
    return False
...
def copyFileToNewDir( fileName, currentSceneName, sourceFileDir, currentSceneDir, basePaletteName, ns ):
    ...
    if not os.path.isfile(newPath) :
        ...
        if not success:
            xglog.XGError(maya.stringTable[ u'y_xgenm_xmaya_xgmExternalAPI.kCannotCopyXgenFile'  ] % (oldPath, newPath) )
            ...
            while line and cmp(line, "endAttrs"):
                if line.startswith("xgDataPath"):
                    ...
                    if xgDataPath.startswith("${PROJECT}"):
                        ...
                        curPalDir = os.path.join(projRoot, "xgen", "collections", encodeNameSpace(palName))

                        srcdir = os.path.join(palPath, preDescription)
                        dstdir = os.path.join(curPalDir, curDescription)
                        xgutil.copyFolder(srcdir, dstdir)
                    else:
                        msg = maya.stringTable[u'y_xgenm_xmaya_xgmExternalAPI.kPalPathError' ]
                        xglog.XGError(msg )
                line = next(fp)
                line = line.strip(' \t\r\n')
...
def importPalette( fileName, deltas, nameSpace="", wrapPatches=True, wrapGuides=True, validator = None, grooming = False, validatePath = True ):
    ...
    if validatePath:
        ...
        for dir in dirs:
            ...
            if not os.path.exists(expandedPath):
                  msg = maya.stringTable[ u'y_xgenm_xmaya_xgmExternalAPI.kxgDataPathBroken' ] % (palName,dir)
                  xglog.XGError(msg )
    ...
    if ( pal == ""):
        ...
        xglog.XGError(maya.stringTable[u'y_xgenm_xmaya_xgmExternalAPI.kXgmExtAPIErrWrapXgenCol' ]+palName)
        xglog.XGError(maya.stringTable[u'y_xgenm_xmaya_xgmExternalAPI.kXgmExtAPIErrWrapXgenDeleteCol' ])
        deletePalette( palName )
        return ""

    if validator:
        # fix the data path of pallete
        dataPath = xg.paletteRootVar() + "/" + encodeNameSpace(palName)
...
def importDescription( palName, fileName, createPal=False, validator=None, grooming=False ):
    ...
    try:
        ...
        pal = ""
    if ( pal == "" ):
        xglog.XGError(errWrapXgenDesc + descName)
        xglog.XGError(errWrapXgenDeleteDesc)
...
def setMessageLevel( type, level ):
    if ( type == "quitOnError" ):
        xglog.XGWarning(maya.stringTable[u'y_xgenm_xmaya_xgmExternalAPI.kXgmExtAPICannotModifyQuitOnError' ])
        return
...
def descriptionGuides( descr ):
    ...
    guides = cmds.listRelatives(descr,ad=True,type='transform',path=True)
    if guides==None:
        return result

    # Internally, guides are stored in patch order. We return the guides in
    # the same order so the index is meaningful outside Maya. Patch are
    # sorted using C strcmp, which is the python default str.__cmp__().
    from collections import defaultdict
    patchGuides = defaultdict(list)
    for guide in guides:
        shape = cmds.listRelatives(guide,ni=True,shapes=True,type=types,path=True)
        if shape:
            patch = guidePatch(guide)
            if patch:
                # See fixPatchNames() below. Patch is post-fixed with description.
                geom = stripNameSpace(str(patch))
                if geom.endswith('_' + descr):
                    geom = geom[:-(len(descr)+1)]
                patchGuides[geom].append(guide)
    for patch, guideBlock in sorted(patchGuides.items()):
        for guide in guideBlock:
            result.append(guide)
    return result
...
def modifyFaceBinding( palette, description, mode="Append",
    ...
    """
    if palette=="":
        xglog.XGError(maya.stringTable[u'y_xgenm_xmaya_xgmExternalAPI.kXgmExtAPINoColSpecifiedForBinding' ])
        return
    if description=="":
        xglog.XGError(maya.stringTable[u'y_xgenm_xmaya_xgmExternalAPI.kXgmExtAPINoDescSpecifiedForBinding' ])
        return
    ...
    try:
        ...
        for desc in descs:
            ...
            for patch in patches:
                ...
                if len(conns) < 1:
                    xglog.XGError(maya.stringTable[u'y_xgenm_xmaya_xgmExternalAPI.kXgmExtAPIErrFindingGeo' ]+patch)
                    continue
                geom = conns[0]
...
def bakedGroomManagerBake( pal, desc, pb=False ):
    ...
    # display results
    print(maya.stringTable[u'y_xgenm_xmaya_xgmExternalAPI.kXgmExtAPIFilesBaked' ])
    print(files)
...
def toQtWidget(controlName):
    ...
    if ptr is not None:
        widget = wrapInstance(long_type(ptr), QWidget)
        return widget
    return None
...
def renamePalette( oldName, newName ):
    ...
    def renamePalettePath( oldName, newName ):
        ...
        xglog.XGWarning(maya.stringTable[ u'y_xgenm_xmaya_xgmExternalAPI.kRenamePaletteFolder'  ] % (newPath,oldPath) )
    ...
    if not result:         
        xglog.XGError(maya.stringTable[ u'y_xgenm_xmaya_xgmExternalAPI.kCannotRenamePalette'  ] % (oldName,newName) )
        raise

    if not validPalette == newName :
        xglog.XGWarning(maya.stringTable[ u'y_xgenm_xmaya_xgmExternalAPI.kPaletteClash'  ] % (newName,validPalette) )
    ...
    try:
        ...
        xglog.XGError(maya.stringTable[ u'y_xgenm_xmaya_xgmExternalAPI.kCannotRenamePalette2'  ] % (oldName,newName) )
        raise
...
def renameDescription( palName, oldName, newName ):
    ...
    def renameDescriptionPath( palName, oldName, newName ):
        ...
        xglog.XGWarning(maya.stringTable[ u'y_xgenm_xmaya_xgmExternalAPI.kRenameDescriptionFolder'  ] % (newPath,oldPath) )
    ...
    cannotRenameDescriptionErr = maya.stringTable[ u'y_xgenm_xmaya_xgmExternalAPI.kCannotRenameDescription'  ] % (oldName,newName) 
    if not result:         
        xglog.XGError(cannotRenameDescriptionErr)
        raise

    if not validDesc == newName :
        xglog.XGWarning(maya.stringTable[ u'y_xgenm_xmaya_xgmExternalAPI.kDescriptionClash'  ] % (newName,validDesc) )
...
def validateDescription( action, palInfo, descInfo, uiParent=None ):
    ...
    if action == ADD_TO_NEW_PALETTE:
        ...
        splitNS = palName.split( ':' )
        for s in splitNS:
            if len(s) == 0:
                title = maya.stringTable[ u'y_xgenm_xmaya_xgmExternalAPI.kCreateXgenDescription'  ]
                text = maya.stringTable[ u'y_xgenm_xmaya_xgmExternalAPI.kInvalidNamespace'  ]
                if uiParent:
                    mbox = QMessageBox(QMessageBox.NoIcon, title, text, QMessageBox.Ok, uiParent )
                    mbox.exec_()
                else:
                    xglog.XGError(text)
                return (False, '','')
    ...
    if selCount and (len(selNS) and len(palNS)) and (selNS != palNS):            
        title = maya.stringTable[ u'y_xgenm_xmaya_xgmExternalAPI.kCreateXgenDescription2'  ]
        text = maya.stringTable[ u'y_xgenm_xmaya_xgmExternalAPI.kCollectionNamespace'  ] % (palNS,selNS)
        if uiParent:
            mbox = QMessageBox(QMessageBox.NoIcon, title, text, QMessageBox.Ok, uiParent )
            mbox.exec_()
        else:
    ...
    if selCount and len(selNS)==0 and len(palNS):
        title = maya.stringTable[ u'y_xgenm_xmaya_xgmExternalAPI.kCreateXgenDescription3'  ]
        text = maya.stringTable[ u'y_xgenm_xmaya_xgmExternalAPI.kSelectedObjectsNotPartOfCollection'  ] % palNS
        if uiParent:
            mbox = QMessageBox(QMessageBox.NoIcon, title, text, QMessageBox.Ok, uiParent )
            mbox.exec_()
        else:
...
def igExportMaps( xdescriptions ):
    ...
    def _hasPtxFile( dir ):
        if os.path.isdir(dir): 
            ...
            ptxfiles = [x for x in files if x.endswith('.ptx')]
            if len(ptxfiles):
                return True
        return False
...
def undoable( name ):
    try:
        ...
        import traceback
        print(maya.stringTable[u'y_xgenm_xmaya_xgmExternalAPI.kXgmExtAPIErrRunningUndoable' ] % (name, e))
        traceback.print_exc()
    finally:
