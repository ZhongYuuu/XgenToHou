...
import xgenm.ui.dialogs.xgSplineToGeometryConvertUI as splineConvertUI
import sys
...
def convertPrimToPolygon(showOption):
    if showOption:
        ...
        splineConvertUI.showConvertWindow(mayaMainWindow)
    else:
        # get option vars and update geometry renderer options
        initSetting = splineConvertUI.GeometryRendererSetting()
        initSetting.loadFromMayaOptionVar()
        
        # convert by running the geometry renderer
        cmd = splineConvertUI.composeConvertCmd(initSetting)
        if len(cmd) > 0:
