...
class ImportFileUI(QDialog):
    ...
    def __init__(self,which):
        ...
        self.setWindowTitle(maya.stringTable[ u'y_xgenm_ui_dialogs_xgImportFile.kImportCollectionsOrDescriptions'  ])
    ...
    def paletteUI(self):
        self.palPart = QWidget()
        vbox = QVBoxLayout()
        QLayoutItem.setAlignment(vbox, QtCore.Qt.AlignTop)
        self.palPart.setLayout(vbox)
        self.palFile = BrowseUI(maya.stringTable[ u'y_xgenm_ui_dialogs_xgImportFile.kFileName'  ],
                                maya.stringTable[ u'y_xgenm_ui_dialogs_xgImportFile.kFileNameAnn'  ],
                                 "","*.xgen","in")
        self.palFile.optionButton.setToolTip(maya.stringTable[ u'y_xgenm_ui_dialogs_xgImportFile.kBrowseForFiles'  ])
        self.palFile.textValue.setText(xg.userRepo())
        vbox.addWidget(self.palFile)
        self.nameSpace = TextUI(maya.stringTable[ u'y_xgenm_ui_dialogs_xgImportFile.kNamespace'  ],
                                maya.stringTable[ u'y_xgenm_ui_dialogs_xgImportFile.kNamespaceAnn'  ],
                                width=200)
        vbox.addWidget(self.nameSpace)
        if xgg.Maya:
            self.palBindings = CheckBoxUI(maya.stringTable[ u'y_xgenm_ui_dialogs_xgImportFile.kIncludePatchBindings'  ],"",
                                          maya.stringTable[ u'y_xgenm_ui_dialogs_xgImportFile.kBindingsAnn'  ])
            self.palBindings.setValue(True)
            vbox.addWidget(self.palBindings)
        self.tabs.addTab(self.palPart,maya.stringTable[ u'y_xgenm_ui_dialogs_xgImportFile.kCollection'  ])
    ...
    def descriptionUI(self):
        ...
        self.descFile = BrowseUI(maya.stringTable[ u'y_xgenm_ui_dialogs_xgImportFile.kFileNameDescFile'  ],
                                 maya.stringTable[ u'y_xgenm_ui_dialogs_xgImportFile.kFileNameAnn2'  ],
                                 "","*.xdsc","in")
        self.descFile.optionButton.setToolTip(maya.stringTable[ u'y_xgenm_ui_dialogs_xgImportFile.kBrowseForFiles2'  ])
        self.descFile.textValue.setText(xg.userRepo())
        vbox.addWidget(self.descFile)

        if xgg.Maya:
            self.descBindings = CheckBoxUI(maya.stringTable[ u'y_xgenm_ui_dialogs_xgImportFile.kBindGeometry'  ],"",
                                       maya.stringTable[ u'y_xgenm_ui_dialogs_xgImportFile.kBindGeometryAnn'  ])
        ...
        self.addDescriptionToRadio = QRadioButton(maya.stringTable[ u'y_xgenm_ui_dialogs_xgImportFile.kAddDescriptionToExistingCollection'  ])
        ...
        self.createNewPaletteRadio = QRadioButton(maya.stringTable[ u'y_xgenm_ui_dialogs_xgImportFile.kCreateNewCollectionNamed'  ])
        ...
        self.tabs.addTab(self.descPart,maya.stringTable[ u'y_xgenm_ui_dialogs_xgImportFile.kDescriptionTab'  ])
    ...
    def buttons(self,layout):
        ...
        self.importButton = QPushButton(maya.stringTable[ u'y_xgenm_ui_dialogs_xgImportFile.kImport'  ])
        self.importButton.setFixedWidth(DpiScale(100))
        self.importButton.setAutoRepeat(False)
        self.importButton.setToolTip(maya.stringTable[ u'y_xgenm_ui_dialogs_xgImportFile.kImportAnn'  ])
        self.connect(self.importButton, QtCore.SIGNAL("clicked()"), self.importCB)
        hbox.addWidget(self.importButton)
        self.cancelButton = QPushButton(maya.stringTable[ u'y_xgenm_ui_dialogs_xgImportFile.kCancel'  ])
        self.cancelButton.setFixedWidth(DpiScale(100))
        self.cancelButton.setAutoRepeat(False)
        self.cancelButton.setToolTip(maya.stringTable[ u'y_xgenm_ui_dialogs_xgImportFile.kCancelAnn'  ])
    ...
    def importCB(self):
        msgImportFailed = maya.stringTable[ u'y_xgenm_ui_dialogs_xgImportFile.kImportFail'  ]
        if self.getType() == 0:
            # Import palette
            xgenFile = self.getPalFile()
            if not ImportFileUI.fileExists(xgenFile,maya.stringTable[ u'y_xgenm_ui_dialogs_xgImportFile.kFileDoesNotExist'  ]):
                return
            ...
            if not ImportFileUI.fileExists(xdscFile,maya.stringTable[ u'y_xgenm_ui_dialogs_xgImportFile.kFileDoesNotExist2'  ]):
                return
            ...
            if len(pal) and xgg.Maya:
                if self.descBindings.value():
                    ...
                    if not (len(geometries) or faces is not None):
                        mb = QMessageBox()
                        mb.setText(maya.stringTable[ u'y_xgenm_ui_dialogs_xgImportFile.kPleaseSelectAPolygonSurfaceTip'  ])
                        mb.exec_()
                        return
            ...
            if (xgg.DescriptionEditor != 0 ):
                ...
                if xg.igDescription(newDesc) and xgg.Maya:
                    ...
                    xgg.DescriptionEditor.activateTab( maya.stringTable[u'y_xgenm_ui_dialogs_xgImportFile.kGrooming' ] )
...
class Validator(object):
    ...
    def onCollidePalette( uiParent, newPalette, palette, palettePath ):
        ...
        title = maya.stringTable[ u'y_xgenm_ui_dialogs_xgImportFile.kImportCollectionsOrDescriptionsValidator'  ]
        text = maya.stringTable[ u'y_xgenm_ui_dialogs_xgImportFile.kCollectionAlreadyExistsOnDisk'  ] % (palette,palettePath)
        mbox = QMessageBox( QMessageBox.NoIcon, title, text, QMessageBox.NoButton, uiParent )
        newPalNameBtn = QPushButton(maya.stringTable[ u'y_xgenm_ui_dialogs_xgImportFile.kCreateInstead'  ] % newPalette)
        mbox.addButton( newPalNameBtn, QMessageBox.YesRole )
        overPalNameBtn = QPushButton(maya.stringTable[ u'y_xgenm_ui_dialogs_xgImportFile.kOverwrite'  ] % palette)
