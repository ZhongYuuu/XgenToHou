...
class ExportFileUI(QDialog):
    ...
    def __init__(self):
        ...
        self.setWindowTitle(maya.stringTable[ u'y_xgenm_ui_dialogs_xgExportFile.kXgenExport'  ])
    ...
    def paletteUI(self):
        ...
        for pal in palettes:
            browsepal = BrowseUI(pal,
                                 maya.stringTable[ u'y_xgenm_ui_dialogs_xgExportFile.kFileNameAnn'   ],
                                 "","*.xgen","out", pal )
            browsepal.optionButton.setToolTip(maya.stringTable[ u'y_xgenm_ui_dialogs_xgExportFile.kBrowseForAFileName1'  ])
            ...
            if multi:
                ...
                self.box.append(QCheckBox())
                self.box[count].setChecked(True)
                self.box[count].setToolTip(maya.stringTable[u'y_xgenm_ui_dialogs_xgExportFile.kExportPaletteAnn1' ] % pal)
                self.palFile[count].layout().addWidget(self.box[count])
            count += 1
        scrollArea = QScrollArea()
        scrollArea.setWidget(self.palPart)
        scrollArea.setWidgetResizable(True)
        self.tabs.addTab(scrollArea,maya.stringTable[ u'y_xgenm_ui_dialogs_xgExportFile.kCollection1'  ])
    ...
    def descriptionUI(self):
        ...
        label = QLabel(maya.stringTable[ u'y_xgenm_ui_dialogs_xgExportFile.kCollection2'  ])
        label.setFixedWidth(labelWidth())
        label.setAlignment(QtCore.Qt.AlignRight|QtCore.Qt.AlignVCenter)
        label.setIndent(DpiScale(10))
        label.setToolTip(maya.stringTable[ u'y_xgenm_ui_dialogs_xgExportFile.kCollectionAnn1'  ])
        hbox.addWidget(label)
        self.palette = _ComboBoxUI()
        self.palette.setMinimumWidth(DpiScale(220))
        self.palette.setToolTip(maya.stringTable[ u'y_xgenm_ui_dialogs_xgExportFile.kCollectionAnn2'  ])
        palettes = xg.palettes()
        for pal in palettes:
        ...
        hbox.setSpacing(DpiScale(3))
        hbox.setContentsMargins(DpiScale(1),DpiScale(1),DpiScale(1),DpiScale(1))
        label = QLabel(maya.stringTable[ u'y_xgenm_ui_dialogs_xgExportFile.kDescription1'  ])
        label.setFixedWidth(labelWidth())
        label.setAlignment(QtCore.Qt.AlignRight|QtCore.Qt.AlignVCenter)
        label.setIndent(DpiScale(10))
        label.setToolTip(maya.stringTable[ u'y_xgenm_ui_dialogs_xgExportFile.kDescriptionAnn1'  ])
        hbox.addWidget(label)
        self.description = _ComboBoxUI()
        self.description.setMinimumWidth(DpiScale(220))
        self.description.setToolTip(maya.stringTable[ u'y_xgenm_ui_dialogs_xgExportFile.kDescriptionToExport'  ])
        ...
        vbox.addWidget(row)
        # row for file name
        self.descFile = BrowseUI(maya.stringTable[ u'y_xgenm_ui_dialogs_xgExportFile.kFileName1'  ],
                                 maya.stringTable[ u'y_xgenm_ui_dialogs_xgExportFile.kFilePathAnn1'  ],
                                 "","*.xdsc","out")
        self.descFile.optionButton.setToolTip(maya.stringTable[ u'y_xgenm_ui_dialogs_xgExportFile.kBrowseForAFileName2'  ])
        self.connect( self.descFile.textValue, QtCore.SIGNAL("editingFinished()"), self.onDescFileSelected )
        vbox.addWidget(self.descFile)
        self.tabs.addTab(self.descPart,maya.stringTable[ u'y_xgenm_ui_dialogs_xgExportFile.kDescription2'  ])
    ...
    def buttons(self,layout):
        ...
        hbox.setContentsMargins(DpiScale(1),DpiScale(1),DpiScale(1),DpiScale(1))
        self.exportButton = QPushButton(maya.stringTable[ u'y_xgenm_ui_dialogs_xgExportFile.kExport'  ])
        self.exportButton.setToolTip(maya.stringTable[ u'y_xgenm_ui_dialogs_xgExportFile.kExportAnn1'  ])
        ...
        self.cancelButton = QPushButton(maya.stringTable[ u'y_xgenm_ui_dialogs_xgExportFile.kCancel'  ])
        self.cancelButton.setFixedWidth(DpiScale(100))
        self.cancelButton.setAutoRepeat(False)
        self.cancelButton.setToolTip(maya.stringTable[ u'y_xgenm_ui_dialogs_xgExportFile.kCancelAnn'  ])
    ...
    def refreshTip(self,index):
        if index:
            self.exportButton.setToolTip(maya.stringTable[ u'y_xgenm_ui_dialogs_xgExportFile.kExportAnn2'  ])
        else:
            if len(xg.palettes())>1:
                self.exportButton.setToolTip(maya.stringTable[ u'y_xgenm_ui_dialogs_xgExportFile.kExportAllAnn'  ])
            else:
                self.exportButton.setToolTip(maya.stringTable[ u'y_xgenm_ui_dialogs_xgExportFile.kExportCollectionAnn2'  ])
...
def exportFile():
    ...
    if len(xg.palettes()) == 0:
        tellem = QMessageBox()
        tellem.setText(maya.stringTable[ u'y_xgenm_ui_dialogs_xgExportFile.kThereAreNoCollectionsToExport'  ])
        tellem.exec_()
        return
    dialog = ExportFileUI()
    result = dialog.exec_()
    if result == QDialog.Accepted:
        ...
        if dialog.getType() == 0:
            ...
            for i in range(count):
                if dialog.getBoxValue(i):
                    ...
                    if len(fileName) and len(pal):
                        ...
                        xglog.XGError(maya.stringTable[ u'y_xgenm_ui_dialogs_xgExportFile.kYouMustSpecifyAFilename'  ])
        else:
            ...
            if len(xdscFile) and len(pal) and len(desc):
                ...
                xglog.XGError(maya.stringTable[ u'y_xgenm_ui_dialogs_xgExportFile.kYouMustSpecifyFilename'  ])
