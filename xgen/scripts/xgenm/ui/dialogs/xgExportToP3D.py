...
class ExportToPaint3DUI(QDialog):
    ...
    def __init__(self):
        ...
        self.setWindowTitle(maya.stringTable[ u'y_xgenm_ui_dialogs_xgExportToP3D.kXgenExportXpd'  ])
        self.setSizeGripEnabled(True)
        self.setMinimumWidth(DpiScale(550))
        layout = QVBoxLayout()
        self.dirName = BrowseUI(maya.stringTable[ u'y_xgenm_ui_dialogs_xgExportToP3D.kXpdDirectory'  ],
                                 maya.stringTable[ u'y_xgenm_ui_dialogs_xgExportToP3D.kXpdDirectoryAnn'  ])
        self.dirName.setValue("p3dGeometry/XPD/")
        layout.addWidget(self.dirName)
        self.curves = CheckBoxUI(maya.stringTable[ u'y_xgenm_ui_dialogs_xgExportToP3D.kIncludeCurve'  ],
                                 maya.stringTable[ u'y_xgenm_ui_dialogs_xgExportToP3D.kIncludeCurveAnn'  ])
        ...
        self.exportButton = QPushButton(maya.stringTable[ u'y_xgenm_ui_dialogs_xgExportToP3D.kExportFile'  ])
        self.exportButton.setFixedWidth(DpiScale(100))
        self.exportButton.setAutoRepeat(False)
        self.exportButton.setToolTip(maya.stringTable[ u'y_xgenm_ui_dialogs_xgExportToP3D.kExportFileAnn'  ])
        self.connect(self.exportButton, QtCore.SIGNAL("clicked()"),
                     self.accept)
        hbox.addWidget(self.exportButton)
        self.cancelButton = QPushButton(maya.stringTable[ u'y_xgenm_ui_dialogs_xgExportToP3D.kCancel'  ])
        self.cancelButton.setFixedWidth(DpiScale(100))
        self.cancelButton.setAutoRepeat(False)
        self.cancelButton.setToolTip(maya.stringTable[ u'y_xgenm_ui_dialogs_xgExportToP3D.kCancelAnn'  ])
    ...
    if result == QDialog.Accepted:
        ...
        if len(xpdDir):
            if xgg.Maya:
                ...
                goPort = QMessageBox()
                goPort.setText(maya.stringTable[ u'y_xgenm_ui_dialogs_xgExportToP3D.kExportToPaint3dOnlyAvailableInMaya'  ])
                goPort.exec_()
