...
from xgenm.ui import xgmSplinePresetUI
...
importPresetDialogTitle = maya.stringTable[ u'y_xgenm_ui_dialogs_xgImportPreset.kImportPresetToSelectedGeometry'  ]

class ImportPresetUI(QWidget):
    ...
    def descriptionUI(self,layout):
        ...
        self.xgpFile = BrowseUI(maya.stringTable[ u'y_xgenm_ui_dialogs_xgImportPreset.kFileNameOfPreset'  ], "", "", "*.xgp", "in")
        ...
        label = QLabel(maya.stringTable[ u'y_xgenm_ui_dialogs_xgImportPreset.kNewDescriptionNameForPreset'  ])
        label.setAlignment(QtCore.Qt.AlignRight|QtCore.Qt.AlignVCenter)
        label.setIndent(DpiScale(10))
        hbox.addWidget(label)
        self.newDescName = QLineEdit()
        rx = QRegExp("[A-Za-z]+[A-Za-z0-9_]*")
        self.newDescName.setValidator(QRegExpValidator(rx,self))
        ...
        self.addDescriptionToRadio = QRadioButton(maya.stringTable[ u'y_xgenm_ui_dialogs_xgImportPreset.kAddPresetToExistingCollection'  ])
        ...
        self.createNewPaletteRadio = QRadioButton(maya.stringTable[ u'y_xgenm_ui_dialogs_xgImportPreset.kCreateNewCollectionForPreset'  ])
        ...
        # guides options
        guideOptionLabel = QLabel(maya.stringTable[ u'y_xgenm_ui_dialogs_xgImportPreset.kGuideOptionForPreset'  ])
        vbox.addWidget(guideOptionLabel)
        rowLayout1 = self._createRowLayout(vbox)
        generateGuideLabel = QLabel(maya.stringTable[ u'y_xgenm_ui_dialogs_xgImportPreset.kGenerateGuides'  ])
        generateGuideLabel.setAlignment(QtCore.Qt.AlignRight|QtCore.Qt.AlignTop)
        generateGuideLabel.setFixedWidth(DpiScale(150))
        rowLayout1.addWidget(generateGuideLabel)
        self.guideOptionGroup = self._createVerticalGroupLayout(rowLayout1)
        self.transferGuide = QRadioButton(maya.stringTable[ u'y_xgenm_ui_dialogs_xgImportPreset.kTransferGuide'  ])
        self.transferGuide.clicked.connect(self.onClickTransferGuide)
        self.guideOptionGroup.addWidget(self.transferGuide)
        self.noGuide = QRadioButton(maya.stringTable[ u'y_xgenm_ui_dialogs_xgImportPreset.kNoGuide'  ])
        ...
        guideTranferLabel = QLabel(maya.stringTable[ u'y_xgenm_ui_dialogs_xgImportPreset.kGuideTranferForPreset'  ])
        guideTranferLabel.setAlignment(QtCore.Qt.AlignRight|QtCore.Qt.AlignTop)
        guideTranferLabel.setFixedWidth(DpiScale(150))
        rowLayout2.addWidget(guideTranferLabel)
        self.guideTransferRadioButtonGroup = self._createVerticalGroupLayout(rowLayout2)
        self.posBasedGuideTransfer = QRadioButton(maya.stringTable[ u'y_xgenm_ui_dialogs_xgImportPreset.kPosBasedGuideTransfer'  ])
        ...
        self.uvBasedGuideTransfer = QRadioButton(maya.stringTable[ u'y_xgenm_ui_dialogs_xgImportPreset.kUVBasedGuideTransfer'  ])
        ...
        guideOrientionTransfer = QLabel(maya.stringTable[ u'y_xgenm_ui_dialogs_xgImportPreset.kGuideOrientionTransfer'  ])
        guideOrientionTransfer.setAlignment(QtCore.Qt.AlignRight|QtCore.Qt.AlignTop)
        guideOrientionTransfer.setFixedWidth(DpiScale(150))
        ...
        self.rotateGuideOrientationToNewNormal = QRadioButton(maya.stringTable[ u'y_xgenm_ui_dialogs_xgImportPreset.kRotateGuideOrientationToNewNormal'  ])
        self.guideOrientationTransferRadioButtonGroup.addWidget(self.rotateGuideOrientationToNewNormal)
        self.keepGuideOrientation = QRadioButton(maya.stringTable[ u'y_xgenm_ui_dialogs_xgImportPreset.kKeepGuideOrientation'  ])
    ...
    def buttons(self,layout):
        ...
        self.importButton = QPushButton(maya.stringTable[ u'y_xgenm_ui_dialogs_xgImportPreset.kImport'  ])
        ...
        self.cancelButton = QPushButton(maya.stringTable[ u'y_xgenm_ui_dialogs_xgImportPreset.kCancelImport'  ])
    ...
    def fileExists( file, msg ):
        ...
        if not os.path.isfile(file):
            QMessageBox.warning(importPresetDialog, importPresetDialogTitle, msg)
            return False
        return True
    ...
    def importGroomableSplines(self, descName, palName, xgpFilePath):
        ...
        if not os.path.isfile(filename):
            ...
            cmds.warning(maya.stringTable[u'y_xgenm_ui_dialogs_xgImportPreset.kMissGroom' ] % (baseName, dirName))
            return
    ...
    def importCB(self):
        ...
        if not ImportPresetUI.fileExists(xgpFileName,maya.stringTable[ u'y_xgenm_ui_dialogs_xgImportPreset.kPresetFileDoesNotExist'  ]):
            return
        ...
        if len(pal) and xgg.Maya:
            ...
            if not (len(geometries) or faces is not None):
                text = maya.stringTable[ u'y_xgenm_ui_dialogs_xgImportPreset.kPleaseSelectAPolygonSurfaceForPresetTip'  ]
        ...
        if not len(newDesc):
            text = maya.stringTable[ u'y_xgenm_ui_dialogs_xgImportPreset.kImportPresetFail'  ]
            QMessageBox.warning(self, importPresetDialogTitle, text)
            self.close()
            return
        ...
        if (xgg.DescriptionEditor != 0 ):
            ...
            if xg.igDescription(newDesc) and xgg.Maya:
                ...
                xgg.DescriptionEditor.activateTab( maya.stringTable[u'y_xgenm_ui_dialogs_xgImportPreset.kGrooming' ] )
...
def getPresetType(filename):
    """Function to get the type of the preset for the given xgp/xgip file.
    """
    if filename.endswith(".xgip"):
        return "Interactive"
    elif filename.endswith(".xgp"):
        try:
            with open(filename, 'r') as groomFile:
...
def importPresetWithPath(value):
    ...
    """
    if value.endswith(".xgip"):
        xgmSplinePresetUI.xgmSplinePresetUIImportWithPath(value)
    elif value.endswith(".xgp"):
        global importPresetDialog
        if importPresetDialog == None:
