...
class BrowseUI(QWidget):
    ...
    def __init__(self,attr,help="",object="",fileType="",inOrOut="in",mainlabel=""):
        ...
        self.setAccessibleName(r'XgBrowseUI')
        ...
        self.textValue.setMinimumWidth(max(DpiScale(50),
            QFontMetrics(self.textValue.font()).width("XGen ${DESC}/${FXMODULE}/ XGen")))
        self.optionButton = QToolButton()
        self.optionButton.setIcon(CreateIcon("xgBrowse.png"))
        self.optionButton.setFixedSize(DpiScale(24),DpiScale(24))
        self.optionButton.setToolTip(maya.stringTable[ u'y_xgenm_ui_widgets_xgBrowseUI.kRaiseFileBrowser'  ])
        # Horizontal layout
        layout = QHBoxLayout()
...
class FileBrowserUI(BrowseUI):
    ...
    def __init__(self,label,help,fileType="",inOrOut="in"):
        ...
        self.setAccessibleName(r'XgFileBrowserUI')
    ...
    def folderExists( folder, desc, uiParent ):
        ...
        if not (os.path.isdir(path) and os.path.exists(path)):
            mb = QMessageBox(uiParent)
            text = maya.stringTable[ u'y_xgenm_ui_widgets_xgBrowseUI.kFolderDoesNotExist' ] % path
            mb.setText(text)
            mb.exec_()
    ...
    def createFolder(folder,desc,uiParent):
        ...
        if not os.path.exists(path):
            title = maya.stringTable[ u'y_xgenm_ui_widgets_xgBrowseUI.kFileBrowserUI'  ]
            text = maya.stringTable[ u'y_xgenm_ui_widgets_xgBrowseUI.kFolderDoesNotExist2' ] % path
            mbox = QMessageBox( QMessageBox.NoIcon, title, text, QMessageBox.NoButton, uiParent )
            newFolderBtn = QPushButton(maya.stringTable[ u'y_xgenm_ui_widgets_xgBrowseUI.kCreateNewFolder'  ])
            mbox.addButton( newFolderBtn, QMessageBox.YesRole )
