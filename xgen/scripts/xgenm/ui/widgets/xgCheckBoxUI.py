...
class CheckBoxUI(QWidget):
    ...
    def __init__(self,mainLabel,attrs,help="",object="",fixed=0, removeSuffix=0, attrlabels=[], startSpacing=0 ):
        ...
        self.setAccessibleName(r'XgCheckBoxUI')
        ...
        if startSpacing:
            layout.addSpacing(DpiScale(startSpacing))
        ...
        for attr in self.attrs:
            newBox = QCheckBox(checkboxSingleLabel) 
            ...
            if multi:
                ...
                if fixed > 0 and attr!=self.attrs[-1]:
                    newBox.setFixedWidth(DpiScale(fixed))
