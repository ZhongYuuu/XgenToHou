...
class StackUI(QWidget):
    ...
    def __init__(self, margin=3):
        ...
        self.setAccessibleName(r'XgStackUI')
        
        layout = QVBoxLayout()
        QLayoutItem.setAlignment(layout, QtCore.Qt.AlignTop)
        layout.setSpacing(DpiScale(0))
        layout.setContentsMargins(DpiScale(margin),DpiScale(margin),DpiScale(margin),DpiScale(margin))
    ...
    def setCurrentByTypeName(self,typename):
        ...
        if indexFound == -1:
            raise ValueError(maya.stringTable[u'y_xgenm_ui_widgets_xgStackUI.kXgStackUITypeNameNotFound' ] % typename)
        else:
