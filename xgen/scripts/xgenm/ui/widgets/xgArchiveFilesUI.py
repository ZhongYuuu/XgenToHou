...
class ArchiveFilesUI(QWidget):
    ...
    def __init__(self,attr,help="",object="",width=0):
        ...
        self.setAccessibleName(r'XgArchiveFilesUI')
        ...
        sliderLayout.setSpacing(DpiScale(3))
        sliderLayout.setContentsMargins(DpiScale(1),DpiScale(1),DpiScale(1),DpiScale(1))
        ...
        self.slider.setToolTip(maya.stringTable[ u'y_xgenm_ui_widgets_xgArchiveFilesUI.kThumbnailSizeAnn'  ])
        self.connect(self.slider, QtCore.SIGNAL("valueChanged(int)"), self.sliderChanged )
        sliderLayout.addWidget( self.slider )

        # Show Thumbnails checkbox
        self.pbMode = QCheckBox(maya.stringTable[ u'y_xgenm_ui_widgets_xgArchiveFilesUI.kList'  ])
        self.pbMode.setToolTip(maya.stringTable[ u'y_xgenm_ui_widgets_xgArchiveFilesUI.kListAnn'  ])
        ...
        if width:
            self.tableView.setFixedWidth(DpiScale(width))
        
        # Some button bellow the file list.
        archiveButLayout = QHBoxLayout()
        archiveButLayout.setSpacing(DpiScale(3))
        archiveButLayout.setContentsMargins(DpiScale(1),DpiScale(1),DpiScale(1),DpiScale(1))
        
        self.pbCreate = QPushButton(maya.stringTable[ u'y_xgenm_ui_widgets_xgArchiveFilesUI.kCreate'  ])
        self.pbCreate.setToolTip(maya.stringTable[ u'y_xgenm_ui_widgets_xgArchiveFilesUI.kCreateAnn'  ])
        ...
        self.pbAdd = QPushButton(maya.stringTable[ u'y_xgenm_ui_widgets_xgArchiveFilesUI.kAdd'  ])
        self.pbAdd.setToolTip(maya.stringTable[ u'y_xgenm_ui_widgets_xgArchiveFilesUI.kAddAnn'  ])
        archiveButLayout.addWidget(self.pbAdd)
        self.connect(self.pbAdd, QtCore.SIGNAL("clicked()"), self.addArchiveDialog)
        
        self.pbEdit = QPushButton(maya.stringTable[ u'y_xgenm_ui_widgets_xgArchiveFilesUI.kEdit'  ])
        self.pbEdit.setToolTip(maya.stringTable[ u'y_xgenm_ui_widgets_xgArchiveFilesUI.kEditAnn'  ])
        archiveButLayout.addWidget(self.pbEdit)
        self.connect(self.pbEdit, QtCore.SIGNAL("clicked()"), self.editArchiveDialog)
        
        self.pbRemove = QPushButton(maya.stringTable[ u'y_xgenm_ui_widgets_xgArchiveFilesUI.kRemove'  ])
        self.pbRemove.setToolTip(maya.stringTable[ u'y_xgenm_ui_widgets_xgArchiveFilesUI.kRemoveAnn'  ])
        ...
        vlayout = QVBoxLayout()
        vlayout.setSpacing(DpiScale(3))
        vlayout.setContentsMargins(DpiScale(1),DpiScale(1),DpiScale(1),DpiScale(1))
    ...
    def handleMenu( self, pos):
        menu = QMenu()
        menu.addAction(maya.stringTable[ u'y_xgenm_ui_widgets_xgArchiveFilesUI.kEditRightClick'  ],self.emitEditMenu)
        menu.addAction(maya.stringTable[ u'y_xgenm_ui_widgets_xgArchiveFilesUI.kRemoveRightClick'  ],self.emitRemoveMenu)
        ret = menu.exec_(QCursor.pos())
    ...
    def sliderChanged(self,v):
        ...
        self.rowSize = v
        self.tableView.setColumnWidth(0,DpiScale(self.rowSize))
        self.tableView.setColumnWidth(1,DpiScale(self.rowSize))
        for i in range( self.tableView.model().rowCount() ):
            self.tableView.setRowHeight( i, DpiScale(self.rowSize) )
        self.fitTableView()
    ...
    def fitTableView( self ):
        ...
        self.tableView.setFixedHeight( DpiScale(totalSize) )
        self.textEdit.setFixedHeight( DpiScale(totalSize)  )
    ...
    def setValue(self,value):
        ...
        for i in range( model.rowCount() ):
            ...
            thumbWidget = QLabel()
            qColor = QColor(g.color[0]*255,g.color[1]*255,g.color[2]*255,255)
            ...
            if len(resolvedThumbnail) and pixmap.load( resolvedThumbnail ):
                ...
                self.drawPixmapRect( QBrush( qColor ), painter, pixmap, s, e )
                painter.end()
            else:
                try:
                    ...
                    painter = QPainter()
                    painter.begin(pixmap)
                    self.drawPixmapRect( QBrush( qColor ), painter, pixmap, 0.0, 1.0 )
                    painter.end()
                except:
                    print(maya.stringTable[ u'y_xgenm_ui_widgets_xgArchiveFilesUI.kCouldntCreateEmptyPixmap'  ])
    ...
    def fileText(self):
        return futureStr(self.textEdit.toPlainText())
    ...
    def addArchiveDialog(self):
        ...
        dialog.setWindowTitle( maya.stringTable[ u'y_xgenm_ui_widgets_xgArchiveFilesUI.kXgenAddArchives'  ] )
        dialog.setNameFilter( "*.xarc" )
    ...
    def addArchives( self, files ):
        ...
        if xgg.Maya:
            ...
            if len(materialFiles):
                ...
                msgbox.setWindowTitle( maya.stringTable[ u'y_xgenm_ui_widgets_xgArchiveFilesUI.kXgenImportMaterials'  ] )

                strFiles = ""
                for m in materialFiles:
                    strFiles += m + "\n"

                msgbox.setText(maya.stringTable[ u'y_xgenm_ui_widgets_xgArchiveFilesUI.kAddedXarcArchives'  ] % strFiles)
                ...
                if msgbox.exec_() == QMessageBox.Yes:
                    for f in materialFiles:
                        ...
                        try:
                            ...
                            print(maya.stringTable[u'y_xgenm_ui_widgets_xgArchiveFilesUI.kErrorWhileReadFile' ])
                            print(str(e))
    ...
    def removeArchiveDialog(self):
        ...
        if len(groups):
            ...
            if self.mode:
                ...
                sel, ok = QInputDialog.getItem(self, maya.stringTable[ u'y_xgenm_ui_widgets_xgArchiveFilesUI.kXgenRemoveArchive'  ], maya.stringTable[ u'y_xgenm_ui_widgets_xgArchiveFilesUI.kArchive'  ], items, current=0, editable=False  )
                if ok:
    ...
    def editArchiveDialog( self ):
        ...
        if len(groups):
            ...
            if self.mode:
                ...
                sel, ok = QInputDialog.getItem(self, maya.stringTable[ u'y_xgenm_ui_widgets_xgArchiveFilesUI.kXgenEditArchive2'  ], maya.stringTable[ u'y_xgenm_ui_widgets_xgArchiveFilesUI.kArchive2'  ], items, current=0, editable=False  )
                if ok:
