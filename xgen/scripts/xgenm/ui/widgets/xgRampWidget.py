...
class RampGraphicsView(QGraphicsView):
...
class CurveScene(QGraphicsScene):
    ...
    def drawRect(self):
        ...
        if not self.baseRect:
            self.baseRect = self.addRect(-1, -1, self.width+2, self.height+2, QPen(QtCore.Qt.black, 1.0), QBrush(QtCore.Qt.gray))
    ...
    def drawPoly(self):
        ...
        if not self.curvePoly:
            self.curvePoly = self.addPolygon(QPolygonF(), QPen(QtCore.Qt.black, 1.0), QBrush(QtCore.Qt.darkGray))

        self.poly = QPolygonF()
        self.poly.append(QPointF(self.width, 0))
        self.poly.append(QPointF(0, 0))
        
        # Use something faster than hardcoded 1000
        subdi = (self.width)//3
        for i in range(0, subdi):
            x = float(i)/float(subdi-1)
            self.poly.append(QPointF(self.width*x, self.height*self.curve.getValue(x)))
        self.poly.append(QPointF(self.width, 0))
    ...
    def drawPoints(self):
        ...
        for i in range(len(self.curve.cv)):
            ...
            if i == self.selectedItem:
                pen = QPen(QtCore.Qt.white,1.0)
            else:
                pen = QPen(QtCore.Qt.black,1.0)
            circle = self.addEllipse(pt[0]*self.width-2, pt[1]*self.height-4, 8, 8, pen, QBrush())
    ...
    def drawDeletePoints(self):
        ...
        for i in range(len(self.curve.cv)):
            ...
            if i == self.selectedItem:
                ...
                pen = QPen(QtCore.Qt.black,1.0)
            rect = self.addRect(pt[0]*self.width-4, -8, 8, 8, pen, QBrush(QtCore.Qt.gray))
            rect.setFlag(QGraphicsItem.ItemIsMovable, True)
            rect.setZValue(self.POINTS_ZORDER)

            Line1 = QLineF(rect.rect().topLeft(), rect.rect().bottomRight())
            Line2 = QLineF(rect.rect().topRight(), rect.rect().bottomLeft())
            crossLine1 = self.addLine(Line1, pen)
            crossLine1.setFlag(QGraphicsItem.ItemIsMovable, True)
            crossLine1.setZValue(self.POINTS_ZORDER)
            crossLine2 = self.addLine(Line2, pen)
            crossLine2.setFlag(QGraphicsItem.ItemIsMovable, True)
...
class xgRampWidget(QWidget):
    ...
    def __init__(self,pLabel=None,vLabel=None,iLabel=None, RampCurve = None,parent=None, FixEditSize = 320):
        ...
        self.setAccessibleName(r'XgRampWidget')

        mainLayout = QVBoxLayout()
        mainLayout.setSpacing(DpiScale(0))
        mainLayout.setContentsMargins(DpiScale(1),DpiScale(1),DpiScale(1),DpiScale(1))

        editor = QWidget()
        editorLayout = QHBoxLayout()
        QLayoutItem.setAlignment(editorLayout, QtCore.Qt.AlignTop|QtCore.Qt.AlignLeft)
        editorLayout.setSpacing(DpiScale(0))
        editorLayout.setContentsMargins(DpiScale(0),DpiScale(0),DpiScale(0),DpiScale(0))
        editor.setLayout(editorLayout)
         
        edits = QWidget()
        editsLayout = QHBoxLayout()
        QLayoutItem.setAlignment(editsLayout, QtCore.Qt.AlignTop|QtCore.Qt.AlignLeft)
        editsLayout.setSpacing(DpiScale(10))
        editsLayout.setContentsMargins(DpiScale(1),DpiScale(1),DpiScale(1),DpiScale(1))
        edits.setLayout(editsLayout)
        edits.setFixedWidth(DpiScale(FixEditSize))

        selPos = QWidget()
        selPosLayout = QHBoxLayout()
        selPosLayout.setSpacing(DpiScale(1))
        selPosLayout.setContentsMargins(DpiScale(1),DpiScale(1),DpiScale(1),DpiScale(1))
        selPos.setLayout(selPosLayout)
        self.selPosEdit = QLineEdit()
        self.validatePos = MayaQclocaleDoubleValidator(0.0,1.0,6,self.selPosEdit)
        self.validatePos.fixup = self.myPosFixup
        self.selPosEdit.setValidator(self.validatePos)
        self.selPosEdit.setFixedWidth(DpiScale(36))
        if pLabel != None:
            selPosLayout.addWidget(QLabel(pLabel))
        else:
            selPosLayout.addWidget(QLabel(maya.stringTable[ u'y_xgenm_ui_widgets_xgRampWidget.kSelectedPosition'  ]))
        selPosLayout.addWidget(self.selPosEdit)
        
        self.PosSpacer = QSpacerItem(DpiScale(1), DpiScale(20), QSizePolicy.Minimum, QSizePolicy.Minimum) #Expanding
        selPosLayout.addItem(self.PosSpacer)
        
        selVal = QWidget()
        selValLayout = QHBoxLayout()
        selValLayout.setSpacing(DpiScale(1))
        selValLayout.setContentsMargins(DpiScale(1),DpiScale(1),DpiScale(1),DpiScale(1))
        selVal.setLayout(selValLayout)
        self.selValEdit = QLineEdit()
        self.validateVal = MayaQclocaleDoubleValidator(0.0,1.0,6,self.selValEdit)
        self.validateVal.fixup = self.myValFixup
        self.selValEdit.setValidator(self.validateVal)
        self.selValEdit.setFixedWidth(DpiScale(36))
        if vLabel != None:
            selValLayout.addWidget(QLabel(vLabel))
        else:
            selValLayout.addWidget(QLabel(maya.stringTable[ u'y_xgenm_ui_widgets_xgRampWidget.kSelectedValue'  ]))
        selValLayout.addWidget(self.selValEdit)
        

        self.ValSpacer = QSpacerItem(DpiScale(1), DpiScale(20), QSizePolicy.Expanding, QSizePolicy.Minimum) #Expanding
        ...
        interpLayout.setSpacing(DpiScale(1))
        interpLayout.setContentsMargins(DpiScale(1),DpiScale(1),DpiScale(1),DpiScale(1))
        interp.setLayout(interpLayout)
        self.interpComboBox = _ComboBoxUI()
        self.interpComboBox.addItem(maya.stringTable[ u'y_xgenm_ui_widgets_xgRampWidget.kFlat'  ])
        self.interpComboBox.addItem(maya.stringTable[ u'y_xgenm_ui_widgets_xgRampWidget.kLinear'  ])
        self.interpComboBox.addItem(maya.stringTable[ u'y_xgenm_ui_widgets_xgRampWidget.kSmooth'  ])
        self.interpComboBox.addItem(maya.stringTable[ u'y_xgenm_ui_widgets_xgRampWidget.kSpline'  ])
        self.interpComboBox.setCurrentIndex(1)
        if vLabel != None:
            interpLayout.addWidget(QLabel(iLabel))
        else:
            interpLayout.addWidget(QLabel(maya.stringTable[ u'y_xgenm_ui_widgets_xgRampWidget.kInterpolation'  ]))
        interpLayout.addWidget(self.interpComboBox)


        self.interpSpacer = QSpacerItem(DpiScale(1), DpiScale(20), QSizePolicy.Expanding, QSizePolicy.Minimum) 
        ...
        self.editorSpacer = QSpacerItem(DpiScale(1), DpiScale(20), QSizePolicy.Expanding, QSizePolicy.Minimum)
        ...
        curveFrameLayout = QHBoxLayout()
        curveFrameLayout.setContentsMargins(DpiScale(1),DpiScale(1),DpiScale(1),DpiScale(1))
