 ...
 * face index table.
 */
struct XGEN_XPDFILE_EXPORT XpdFileHeader
{
public:

    /** Constructor */
    XpdFileHeader() : primSize(0), faceid(0), numPrims(0), blockPosition(0) {}
...
// FileHeader is renamed to XpdFileHeader to avoid global symbol clash.
using FileHeader = XpdFileHeader;

/**
* @brief  A comparer to be used in STD::lower_bound algorithm
*
* lower_bound is a binary search algorithm which must receive a sorted collection
* faceId array in FileHeader is sorted but it completes empty spaces at the end with -1
* FaceIdoComparer is intended to be used as lower_bound comparer parameter
* and it checks -1 value before comparation
*/
struct FaceIdComparer
{
	bool operator() (const int &current, const int &faceId) {
...
protected:
    ...
    XpdFileHeader            _fileHeader;    //  Our file header
