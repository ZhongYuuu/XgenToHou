...
// Cleanup a filename to ensure it is valid for use in the system.
inline bool fileCleanup( std::string &filename );
...
inline bool fileCleanup( std::string &filename )
{
    ...
    // Replace "${x}" or "$x " with getenv(x).
    // Allow \x to mean x (so \$ gets you $).
    //
    // Simple hand-built regexp parser:
    // State 0: normal state.
    // State 1: previous char was a \
    // State 2: previous char was $
    // State 3: in a ${ looking for }
    // State 4: in a $xyz looking for non-alnum.
    //
    // We copy characters into 'cleaned'
    int state = 0;
    for(size_t i = 0; i < filename.length(); ++i) {
        char nextchar = filename[i];
        switch(state) {
            case 0:
                switch(nextchar) {
                    case '\\':
            ...
            case 2:
                if (nextchar == '{') {
                    ...
                    // now we read "$x" and we have started an ennvar
                    state = 4;
                    envvar.push_back(nextchar);
                    continue;
                } else {
                    // now we read "$ "
                    cleaned.push_back('$');
            ...
            case 3:
                if (nextchar != '}') {
                    ...
                    // eat the close-brace, expand the envvar, and start again
                    const char *name = getenv(envvar.c_str());
                    if (name) {
                        cleaned += name;
                    }
                    else {
            ...
            case 4:
                if (isalnum(nextchar) || nextchar == '_') {
                    ...
                    // expand the envvar, then start again at the same position
                    const char *name = getenv(envvar.c_str());
                    if (name) {
                        cleaned += name;
                    }
    ...
    for(;;)
    {
        size_t index0 = cleaned.find( "/.." );
        if( index0 != std::string::npos && index0 > 0 )
        {
            size_t index1 = cleaned.rfind( "/", index0-1 );
            if( index1 != std::string::npos )
            {
                cleaned = cleaned.substr(0,index1) + cleaned.substr(index0+3);
                continue;
            }
        }
    ...
    // If the string starts and end with quotes strip them.
