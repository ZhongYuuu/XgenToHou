...
public:
	...
	// Delete temporary render palettes that are created in init() method.
	// Temporary render palettes are prefixed with "XG_RENDER_".
	static void deleteTempRenderPalettes();
...
#define XGEN_RENDER_API_PARALLEL 1
#ifdef XGEN_RENDER_API_PARALLEL
// Parallel rendering support. This class enables the client code to render
// faces in parallel. ParallelRenderer will call the render() method from
// different threads concurrently.
/*
// API Usage pseudo code.
void Procedural( const char* in_xgen_args )
{
  static MyRendererCallbacks cb;
  bbox faceBox;
  unsigned int faceId = -1;
  ...
  PatchRenderer* patch = PatchRenderer::init( &cb, in_xgen_args );
  ParallelRenderer* par = ParallelRenderer::init( patch );

  while ( patch->nextFace( faceBox, faceId ) )
  {
    FaceRenderer* face = FaceRenderer::init( patch, faceId );
    par->enqueue(face);
  }
...
*/
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
interface XGEN_EXPORT ParallelRenderer
{
public:
	interface_methods( ParallelRenderer );

	// Init a new parallel renderer for the specified patch.
	static ParallelRenderer* init( PatchRenderer* in_patch );

	// canRunInParallel
	//  Return true if the patch can be rendered in parallel. When there are
	//  customized generator or fx modules, they must support parallel as well.
	//  Currently, parallel is turned off when there are motion samples.
	//  Related to XgFXModule::isThreadSafe() method.
	virtual bool canRunInParallel() = 0;

	// setThreadCount
	//  Set the number of rendering threads. If the number of threads is 0,
	//  the renderer will determine the thread count automatically (default).
	//  If the number of threads is -1, parallel rendering is disabled.
	//  Related to tbb::global_control() class.
	virtual void setThreadCount( int in_threads ) = 0;

	// setParallelFlush
	//  Set whether to call flush() in parallel. By default, flush() is called
	//  by only one thread at the same time to help porting.
	virtual void setParallelFlush( bool in_parallel ) = 0;

	// enqueue
	//  Add the face for parallel rendering.
	virtual bool enqueue( FaceRenderer* in_face ) = 0;

	// spawnAndWait
	//  Render the faces in parallel. This implicitly calls the render() method
	//  of FaceRenderer.
	virtual void spawnAndWait() = 0;

	// destroy
	//  A convenient method to delete patch, face and parallel renderers. All
	//  pointers will be invalidated.
	virtual void destroy() = 0;
};
#endif // XGEN_RENDER_API_PARALLEL
