...
#ifndef XGEN_FORCEINLINE
    #if defined(_MSC_VER) && !defined(NDEBUG)
        #define XGEN_FORCEINLINE __forceinline
    #elif defined(__GNUC__) && __GNUC__ >= 4 && !defined(NDEBUG)
        #define XGEN_FORCEINLINE __attribute__((always_inline))
    #else
        #define XGEN_FORCEINLINE inline
    #endif
#endif // XGEN_FORCEINLINE
...
namespace xgutil
{
    ...
    // The implementation is std::mt19937 but I prefer to keep
    // it opaque here.
    ...
    /** get angel of line P-p1 and line P-p2 */
    extern double getAngelOfTwoLine(const SgVec3d& startP, const SgVec3d& p1, const SgVec3d& p2);

    /** convert point cv from one coordinate system to another coordinate system. */
    extern SgVec3d convertAxisCoordinate(const SgVec3d& cv,
        const SgVec3d& orgP, const SgVec3d& orgU, const SgVec3d& orgN, const SgVec3d& orgV,
        const SgVec3d& P, const SgVec3d& U, const SgVec3d& N, const SgVec3d& V);

    /**
     * get point on normal n which has the same distance to point p and pOnN
     *
     * @param p     point not on the normal
     * @param pOnN  point on the normal
     * @param n     normal
     */
    extern SgVec3d getCurveCenter(SgVec3d& p, SgVec3d& pOnN, SgVec3d& n);
    ...
    /* str.partition, return tuple(0) */
    inline std::string partition0(const std::string& str, const std::string& sep)
    {
    ...
    extern XGEN_EXPORT safevector<SgVec3f> changeFrame(
        const safevector<SgVec3f> &start,
        const SgVec3f &s_P,
        const SgVec3f &s_N,
        const SgVec3f &s_U,
        const SgVec3f &s_V,
        safevector<SgVec3f> &finish,
        const SgVec3f &f_P,
        const SgVec3f &f_N,
        const SgVec3f &f_U,
        const SgVec3f &f_V);
...
/** Lowest file version of the file being imported. */
XGEN_EXPORT extern unsigned int XGLowestImportFileVersion;
